...
test("CLI rejects invalid lookup limits", async () => {
  await withTempFixture("code-repo", async (projectRoot) => {
    ...
    await assert.rejects(
      () => execFileAsync(process.execPath, [cliPath, "lookup", "checkout", "--limit", "abc"], { cwd: projectRoot }),
      /must be a positive integer/,
    );

    await assert.rejects(
      () => execFileAsync(process.execPath, [cliPath, "lookup", "checkout", "--limit", "0"], { cwd: projectRoot }),
      /must be a positive integer/,
    );
  });
});
